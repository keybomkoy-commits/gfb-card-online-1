...
export const GameProvider = ({ children }) => {
  ...
  const [mails, setMails] = useState(() => {
    if (isConfigured && db) return [];
    const saved = localStorage.getItem('gfb_mails_v4');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  useEffect(() => {
    ...
    // 6. Sync global mails collection
    const unsubMails = onSnapshot(
      collection(db, 'mails'),
      (snapshot) => {
        const list = [];
        snapshot.forEach(docSnap => {
          const mail = { id: docSnap.id, ...docSnap.data() };
          // Check for expired mails and delete them in cloud background
          if (mail.expiresAt && mail.expiresAt < Date.now()) {
            deleteDoc(doc(db, 'mails', mail.id)).catch(err => console.warn(err));
          } else {
            list.push(mail);
          }
        });
      ...
      (err) => {
        console.error("Firebase mails sync error:", err);
      }
    );
  ...
  useEffect(() => {
    ...
    const handleStorageSync = (e) => {
      try {
        ...
        if (e.key === 'gfb_mails_v4' && e.newValue) {
          setMails(JSON.parse(e.newValue));
        }
      } catch (err) {
  ...
  const sendMail = async (mailData) => {
    const newMail = {
      id: `mail_${Date.now()}_${generateUniqueId('rand')}`,
      title: mailData.title || 'ประกาศสโมสร',
      message: mailData.message || '',
      sender: 'admin',
      targetType: mailData.targetType || 'all',
      targetUser: mailData.targetUser ? mailData.targetUser.trim().toLowerCase() : null,
      giftCoins: parseInt(mailData.giftCoins) || 0,
      giftCardTemplateId: mailData.giftCardTemplateId || null,
      createdAt: new Date().toISOString(),
      expiryType: mailData.expiryType || 'permanent',
      expiresAt: mailData.expiresAt || null,
      claimedBy: [],
      readBy: [],
      deletedBy: []
    };
    ...
    if (isConfigured && db && firebaseActive) {
      try {
        ...
        console.error("Firebase sendMail failed", err);
      }
    } else {
      setMails(prev => {
        const updated = [newMail, ...prev];
        localStorage.setItem('gfb_mails_v4', JSON.stringify(updated));
        return updated;
      });
    }
    return { success: true };
  ...
  const claimMailAttachment = async (mailId) => {
    if (!currentUser) return { success: false, message: 'กรุณาเข้าสู่ระบบ' };

    const mail = mails.find(m => m.id === mailId);
    if (!mail) return { success: false, message: 'ไม่พบจดหมายนี้' };

    if (mail.claimedBy?.includes(currentUser.username)) {
      return { success: false, message: 'คุณเคยเคลมรางวัลของจดหมายฉบับนี้ไปแล้ว' };
    }

    if (mail.expiresAt && mail.expiresAt < Date.now()) {
      return { success: false, message: 'จดหมายนี้หมดอายุแล้ว' };
    }

    let newCards = [...currentUser.cards];
    let newCoins = currentUser.coins;

    if (mail.giftCoins > 0) {
      newCoins += mail.giftCoins;
    }

    if (mail.giftCardTemplateId) {
      const baseCard = cardsDB.find(c => c.id === mail.giftCardTemplateId);
      if (baseCard) {
        newCards.push({
          ...baseCard,
          instanceId: generateUniqueId('gift'),
          suspensionRemaining: 0,
          injuredUntil: null
        });
      } else {
        return { success: false, message: 'ไม่พบฐานข้อมูลแม่แบบนักเตะใบนี้แล้ว' };
      }
    }
    ...
    const updatedClaimedBy = [...(mail.claimedBy || []), currentUser.username];
    if (isConfigured && db && firebaseActive) {
      try {
        await setDoc(doc(db, 'mails', mailId), { claimedBy: updatedClaimedBy }, { merge: true });
      } catch (err) {
        console.error("Firebase claimMail failed", err);
      }
    } else {
      setMails(prev => {
        const updated = prev.map(m => m.id === mailId ? { ...m, claimedBy: updatedClaimedBy } : m);
        localStorage.setItem('gfb_mails_v4', JSON.stringify(updated));
        return updated;
      });
    }

    return { success: true, message: 'กดเคลมรับของรางวัลและนักเตะเข้าสู่สโมสรเรียบร้อย!' };
  };

  const readMail = async (mailId) => {
    if (!currentUser) return;
    const mail = mails.find(m => m.id === mailId);
    if (!mail || mail.readBy?.includes(currentUser.username)) return;

    const updatedReadBy = [...(mail.readBy || []), currentUser.username];

    if (isConfigured && db && firebaseActive) {
      try {
        await setDoc(doc(db, 'mails', mailId), { readBy: updatedReadBy }, { merge: true });
      } catch (err) {
        console.error("Firebase readMail failed", err);
      }
    } else {
      setMails(prev => {
        const updated = prev.map(m => m.id === mailId ? { ...m, readBy: updatedReadBy } : m);
        localStorage.setItem('gfb_mails_v4', JSON.stringify(updated));
        return updated;
      });
    }
  ...
  const deleteMail = async (mailId) => {
    ...
    if (mail.targetType === 'user') {
      if (isConfigured && db && firebaseActive) {
        try {
          await deleteDoc(doc(db, 'mails', mailId));
        } catch (err) {
          console.error("Firebase deleteMail failed", err);
        }
      } else {
        setMails(prev => {
          const updated = prev.filter(m => m.id !== mailId);
          localStorage.setItem('gfb_mails_v4', JSON.stringify(updated));
          return updated;
        });
      }
    } else {
      const updatedDeletedBy = [...(mail.deletedBy || []), currentUser.username];
      if (isConfigured && db && firebaseActive) {
        try {
          await setDoc(doc(db, 'mails', mailId), { deletedBy: updatedDeletedBy }, { merge: true });
        } catch (err) {
          console.error("Firebase hideMail failed", err);
        }
      } else {
        setMails(prev => {
          const updated = prev.map(m => m.id === mailId ? { ...m, deletedBy: updatedDeletedBy } : m);
          localStorage.setItem('gfb_mails_v4', JSON.stringify(updated));
          return updated;
        });
      }
