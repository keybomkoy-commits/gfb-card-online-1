...
import { LayoutDashboard, Users, ShoppingBag, Swords, ShieldCheck, LogOut, Coins, Trophy, Mail } from 'lucide-react';

export const Navbar = ({ currentView, setView, onMailboxOpen }) => {
  const { currentUser, logout, mails, firebaseActive } = useGame();
  ...
  // Calculate unread/unclaimed mails count
  const getUnreadMailsCount = () => {
    if (!mails) return 0;
    return mails.filter(mail => {
      ...
      if (mail.expiresAt && mail.expiresAt < Date.now()) return false;
      ...
      return !isRead || (isGift && !isClaimed);
    }).length;
  };

  const unreadCount = getUnreadMailsCount();
  ...
  return (
    <header className="sticky top-0 w-full z-40 px-4 pt-4 select-none">
      <div className="max-w-7xl mx-auto glass-premium flex items-center justify-between px-6 py-4 border border-white/5 shadow-2xl">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Mailbox trigger button */}
          <button
            onClick={onMailboxOpen}
            className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-800 text-slate-400 hover:text-emerald-400 hover:border-emerald-500/25 transition-all relative flex items-center justify-center shrink-0"
            title="กล่องจดหมายสโมสร"
          >
            <Mail size={16} />
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-rose-500 text-white text-[8px] font-black flex items-center justify-center border border-slate-950 animate-pulse">
                {unreadCount}
              </span>
            )}
          </button>
